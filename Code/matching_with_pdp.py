...
class FingerprintDB:
    ...
    #             for song_id, st in self.db[h]:
    #                 offset = round(st - qt, 2) 
    #                 if (song_id, offset) in dict_match:
    #                     dict_match[(song_id, offset)]+=1
    #                 else:
    #                     dict_match[(song_id, offset)]=1
    #                 if dict_match[(song_id, offset)]>got_max:
    #                     got_max=dict_match[(song_id, offset)]
    #                     song_max=song_id
    #     return song_max,got_max
...
def recognize_uploaded_song(file_path="../query/jeena_jeena_recording.mp3"):
    ...
    # items = [(song_id, path) for song_id, path in songs.items()
    #      if song_id not in saved_hashes]

    # with Pool(4) as p:
    #     results = p.map(_process_song, items)

    # for song_id, hashes, err in results:
